 ...
 * time-trap in `isSpam`. Comment spam is overwhelmingly link injection, so a URL
 * in the name field, or more than `MAX_LINKS` links in the body, is treated as
 * spam. The caller drops these silently (nothing stored, no signal to the bot).
 */
export function looksSpammy(fields = {}) {
  const name = String(fields.name || '');
  const body = String(fields.comment || '');
  if (URL_IN_TEXT.test(name)) return true;
  return (body.match(URL_IN_TEXT_G) || []).length > MAX_LINKS;
}
