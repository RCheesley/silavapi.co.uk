...
export function renderGallery(items = []) {
  ...
  const figures = list
    .map((it) => {
      ...
      // …"); the figcaption is shown when either is present.
      const captionText = it.caption && String(it.caption).trim() ? escapeHtml(it.caption) : '';
      const creditText =
        it.credit && String(it.credit).trim()
          ? `<span class="gallery__credit">${escapeHtml(it.credit)}</span>`
          : '';
      const caption =
