 ...
 * alt text (and an optional visible caption and/or photo credit). Each image links to its
...
/**
 * @param {Array<{src:string, alt?:string, caption?:string, credit?:string}>} items
...
export function renderGallery(items = []) {
  ...
  const figures = list
    .map((it) => {
      ...
      // A figure can carry a visible caption and/or a credit (e.g. "Photo by
      // …"); the figcaption is shown when either is present. Trim before
      // rendering so stray whitespace can't leak in or double-space the join.
      const captionRaw = String(it.caption || '').trim();
      const creditRaw = String(it.credit || '').trim();
      const captionText = captionRaw ? escapeHtml(captionRaw) : '';
      const creditText = creditRaw
        ? `<span class="gallery__credit">${escapeHtml(creditRaw)}</span>`
        : '';
      const caption =
        captionText || creditText
          ? `<figcaption class="gallery__caption">${captionText}` +
            `${captionText && creditText ? ' ' : ''}${creditText}</figcaption>`
          : '';
