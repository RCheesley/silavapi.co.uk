...
/** Count URL-like tokens in a string (http(s):// or bare www.domain). */
function countLinks(value) {
  const m = String(value || '').match(/https?:\/\/|www\.[a-z0-9-]+\.[a-z]/gi);
  return m ? m.length : 0;
}
...
// them), because a match is dropped silently.
export const MAX_MESSAGE_LINKS = 5;

/**
 * Content heuristics for spam that passes the honeypot and time-trap. Checks the
 * free-text fields of both the contact and speaker forms.
...
export function looksLikeSpamContent(fields = {}) {
  ...
  // BBCode or HTML link markup never appears in a genuine plain-text message.
  if (/\[url[=\]]|\[link[=\]]|<a\s+href=/i.test(body)) return true;
  // A pile of links in a short contact message is spam.
  if (countLinks(body) >= MAX_MESSAGE_LINKS) return true;
