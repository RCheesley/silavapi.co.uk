...
// Every list-directed alternation requires a real list keyword after the verb,
// so "subscribe to your newsletter", "join your mailing list" and "be added to
// your database" match while "subscribe to your RSS feed" / "…your blog" (a
// genuine non-list target) do not. Because the regex is unanchored, the "I'd
// like to …" preamble is caught for free (the "subscribe to your <list>"
// substring still matches). Standalone imperatives ("subscribe me", "sign me
// up") and the "let me know when I'm subscribed" confirmation-chaser round it
// out. The leading lookbehind exempts a negator immediately before the trigger,
// so a genuine opt-out ("do not subscribe me", "don't add my email…") is not
// flagged.
const LIST_SPAM_RE = new RegExp(
  `(?<!\\b${NEGATOR}\\s{1,4})\\b(?:` +
    ...
    `(?:be added|be subscribed|be signed up) to (?:your |the )?${LIST_CONTEXT}|` +
